...
public class Zombies_ extends PApplet {




public boolean hit(float x, float y, float x1, float y1, float x2, float y2) {
...
public void zombie(){//auto places a zombie with 100 health at a random wall at a random point on that wall
  float[] xyh=new float[4];{//used to get x,y,health,speed of zombies
    xyh[2]=100;//health at 100%
    xyh[3]=0;
  }
  boolean one=random(1)<.5f;//gets random bool
  boolean two=random(1)<.5f;//random bool 2
...
public void mine(float x,float y,ArrayList<float[]>mines){
...
public void rocket(float x, float y, float angle, ArrayList<float[]> rockets){
  float[] xya={x,y,angle,30};
  rockets.add(xya);
}
public ArrayList <float[]> boom(float x,float y,ArrayList<float[]>zombies){
  explotion.play();
  fill(255,255,0);
  ellipse(x,y,100,100);
...
public void zombie(float x, float y, int health){//un used overload to manually place zombie at an x and y with health
  float[] xyh=new float[4];
  xyh[0]=x;
  xyh[1]=y;
  xyh[2]=health;
...
public boolean button(int x1,int y1,int x2,int y2, int mouseX, int mouseY){
  boolean returns=(((x1<mouseX&&mouseX<x2)||(x2<mouseX&&mouseX<x2))&&((y1<mouseX&&mouseX<y2)||(y1<mouseY&&mouseY<y2)));
  return returns;
}
public ArrayList<float[]> bullet(float x, float y, float angle,float damage, ArrayList<float[]>zombies) {//everything that is run when firing
...
public void drawLegs (float x, float y, float angle ,float frame){
...
public void keyPressed() {//sets bool to true if the key is pressed
...
public void keyReleased() {//if a key is released then set its bool to false
...
SoundFile explotion;
public void setup() {
  //screen size, everything should be scalable with height and width
  frameRate(40);
  textSize(15);
  for(int i=0;i<15;i++){
    zombie();
...
public void draw() {
  ...
  if(menu){
    ...
    if (mousePressed){
      ...
      if(fireRateTimer<=0){
         ...
         fireRateTimer=PApplet.parseInt(guns[gs][1]);
      }
    }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Zombies_" };
    if (passedArgs != null) {
